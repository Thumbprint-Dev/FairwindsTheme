...
// Local development server for the Fairwinds storefront theme.
//
// Serves this repo's app/ folder -- the same folder Four51 deploys -- and
// forwards /api/* to the real Fairwinds server, so you can edit a partial or
// custom.css and see the change on refresh without deploying a commit.
//
//   node devserver.mjs
//   node devserver.mjs --port 8080
//   node devserver.mjs --upstream https://thumbprint.Four51OrderCloud.com/fairwinds
//
...
// This theme drives exactly one storefront. Use --upstream to aim at the older
// /fairwinds site or a sandbox clone without editing this.
const SITE = {
	name: 'FairwindsV2',
	url: 'https://thumbprint.Four51OrderCloud.com/FairwindsV2'
}
...
Options:
  ...
  -u, --upstream <url> Override the API target (e.g. the older /fairwinds site
                       or a sandbox). Defaults to ${SITE.url}
